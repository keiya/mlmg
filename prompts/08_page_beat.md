...
セリフは **短く・口語的に・1 セリフ 30 文字以内** に抑えてください（吹き出しに収まるように）。長い説明は **ナレーション枠** (`narration` bubble_type) を積極的に使い、状況・時間経過・心情の exposition を担わせてください。ナレーションも 1 枠 30 文字以内、必要なら同一コマに複数置いてよい。

セリフ数の上限はありません — ページの感情密度に応じて必要な数だけ置いてください。ただし「dialogue / inner_monologue / narration」を **目的別に使い分ける** こと:
- `dialogue`: キャラ間の対話
- `inner_monologue`: 心の声、口に出してない思考
- `narration`: 場面の説明、時間経過、状況の客観描写（読者向けの exposition）
...
- Speech 数に上限は設けない。コマの感情密度・必要な exposition に応じて自由に使う。`narration` を場面説明・時間経過・心情に積極的に活用する
